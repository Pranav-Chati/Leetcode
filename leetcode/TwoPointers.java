...
public class TwoPointers {
    ...
    public static int tripletSumCloseToTarget(int[] nums, int target) {
        ...
        for (int i = 0; i < nums.length - 2; i++) {
            ...
            while (left < right) {
                ...
                if (Math.abs(diff) < Math.abs(closetDiff))
                    closetDiff = diff;
    ...
    public static void dutchNationalProblem(int[] nums) {
        ...
        while (zeroPointer <= index && index <= twosPointer) {
            if (nums[index] == 0) {
                swap(nums, index, zeroPointer);
                zeroPointer++;
                index++;
            } else if (nums[index] == 1) {
                index++;
            } else {
                swap(nums, index, twosPointer);
                twosPointer--;
            }
        }
    }
